...
public class ObjectRecognitionModelTrainer(IImageConverter imageConverter) : BaseMl
{
    // Training Variables
    ...
    public (bool Success, string Exception) Train()
    {
        try
        {
            ...
            IEstimator<ITransformer> pipeline = MlContext.Transforms.LoadImages(outputColumnName: INPUT, imageFolder: _imagesFolder, inputColumnName: nameof(TrainerImageDataInputItem.ImagePath))
            ...
            IDataView trainingData = MlContext.Data.LoadFromTextFile<TrainerImageDataInputItem>(path: _trainTagsTsv, hasHeader: false);
    ...
    public (bool Success, string Exception) Train2()
    {
        try
        {
            if (File.Exists(ML_NET_MODEL))
            ...
            IEstimator<ITransformer> pipeline = MlContext.Transforms.LoadRawImageBytes(outputColumnName: INPUT, imageFolder: _imagesFolder, inputColumnName: nameof(ImageDataInput.Bitmap))
                .Append(MlContext.Transforms.ResizeImages(outputColumnName: INPUT, imageWidth: InceptionSettings.ImageWidth, imageHeight: InceptionSettings.ImageHeight, inputColumnName: INPUT))
                .Append(MlContext.Transforms.ExtractPixels(outputColumnName: INPUT, interleavePixelColors: InceptionSettings.ChannelsLast, offsetImage: InceptionSettings.Mean))
                .Append(MlContext.Model.LoadTensorFlowModel(_inceptionTensorFlowModel)
                .ScoreTensorFlowModel(outputColumnNames: new[] { TF_SOFTMAX }, inputColumnNames: new[] { INPUT }, addBatchDimensionInput: true))
                .Append(MlContext.Transforms.Conversion.MapValueToKey(outputColumnName: "LabelKey", inputColumnName: nameof(ImageDataPredictionItem2.Label)))
                .Append(MlContext.MulticlassClassification.Trainers.LbfgsMaximumEntropy(labelColumnName: "LabelKey", featureColumnName: TF_SOFTMAX))
                .Append(MlContext.Transforms.Conversion.MapKeyToValue(nameof(ImageDataPredictionItem2.PredictedLabelValue), "PredictedLabel"))
                .AppendCacheCheckpoint(MlContext);

            IDataView trainingData = MlContext.Data.LoadFromTextFile<ImageDataInput>(path: _trainTagsTsv, hasHeader: false);
